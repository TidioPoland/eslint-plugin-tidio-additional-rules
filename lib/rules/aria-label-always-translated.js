        ...
        if (node.value.type.toLowerCase() === 'literal') {
          ...
          for (
            ...
            if (
              ...
              context.report({
                node: node,
                message: `aria-label should always be translated. Got a function "${expression.callee.name}" call in string template. Please use trans directly.`,
              });
              return undefined;
            }
            if (expression.type.toLowerCase() === 'identifier') {
              context.report({
                node: node,
                message: `aria-label should always be translated. Got a variable "${expression.name}" call in string template. Please use trans directly.`,
              });
              return undefined;
            }
          }
          for (let i = 0; i < node.value.expression.quasis.length; i += 1) {
            ...
            if (quasi.value.raw.match(/[a-zA-Z]/gi)) {
              context.report({
                node: node,
                message: `aria-label should always be translated. Got an untranslated "${quasi.value.raw.trim()}" string in string template. Please use trans.`,
              });
              return undefined;
            }
          }
