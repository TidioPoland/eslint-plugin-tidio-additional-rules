    ...
    return {
      JSXAttribute(node) {
        ...
        if (node.value.type.toLowerCase() === 'literal') {
          ...
          node.value.type.toLowerCase() === 'jsxexpressioncontainer' &&
          node.value.expression.type.toLowerCase() === 'identifier'
        ) {
          context.report({
            node: node,
            message: `aria-label should always be translated. Got a variable "${node.value.expression.name}". Please use trans directly.`,
          });
        }
      },
    };
